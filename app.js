...
var platformHints = {
  ...
  bilibili:    '🔵 <b>Bilibili</b> — Paste any bilibili.com or bilibili.tv video link. Embed sync support.',
  vimeo:       '🔵 <b>Vimeo</b> — Paste any vimeo.com video link. Embed sync support.',
  dailymotion: '🔵 <b>Dailymotion</b> — Paste any dailymotion.com video link. Embed sync support.',
  twitch:      '🟣 <b>Twitch VOD</b> — Paste a twitch.tv/videos/... link. Embed sync support.',
  odysee:      '🔴 <b>Odysee</b> — Paste any odysee.com video link. Embed sync support.',
  ...
  drive:       '🟢 <b>Google Drive</b> — Share → Anyone with link → Copy link → paste here. Sync may be limited.',
  mp4:         '▤ <b>Direct .mp4</b> — Any public direct video URL ending in .mp4, .webm, or .ogg. Full sync support.'
};
...
function doLoadVideo(url, pushToRoom) {
  ...
  if (/youtube\.com|youtu\.be/.test(url)) {
    ...
    type = 'embed';
    var frame = getEl('videoFrame');
    // Dailymotion updated their embed URL format
    ...
    if (/bilibili\.tv/.test(url)) {
      // International Bilibili TV — e.g. bilibili.tv/en/video/2015598657
      ...
      // Bilibili anime/drama episode or series
      var bangumi = url.match(/\/bangumi\/play\/(ep|ss)(\d+)/);
      biliSrc = 'https://player.bilibili.com/player.html?' + bangumi[1] + 'id=' + bangumi[2] + '&autoplay=0';
    } else {
      // Regular bilibili.com video BV or av
      var bvid = url.match(/video\/(BV[a-zA-Z0-9]+|av\d+)/);
      if (!bvid) { alert('Could not parse Bilibili video ID.'); return; }
      biliSrc = 'https://player.bilibili.com/player.html?bvid=' + bvid[1] + '&autoplay=0';
    }
    ...
    var nicoId = url.match(/watch\/(sm\d+|\d+)/);
    if (!nicoId) { alert('Could not parse NicoNico video ID.'); return; }
    type = 'embed';
    var frame = getEl('videoFrame');
    frame.src = 'https://embed.nicovideo.jp/watch/' + nicoId[1] + '?autoplay=0';
    frame.style.display = 'block';

  } else if (/odysee\.com|lbry\.tv/.test(url)) {
    type = 'embed';
    var frame = getEl('videoFrame');
    var odyseeEmbed = url.replace('odysee.com/', 'odysee.com/$/embed/').replace('lbry.tv/', 'odysee.com/$/embed/');
    frame.src = odyseeEmbed;
    frame.style.display = 'block';
