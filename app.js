...
var platformHints = {
  ...
  bilibili:    '🔵 <b>Bilibili</b> — Paste any bilibili.com/video/BV... link. Embed sync support.',
...
function doLoadVideo(url, pushToRoom) {
  ...
  if (/youtube\.com|youtu\.be/.test(url)) {
    ...
    var biliId = url.match(/video\/(BV[a-zA-Z0-9]+|av\d+)/);
    if (!biliId) { alert('Could not parse Bilibili video ID.'); return; }
    type = 'embed';
    var bvid = biliId[1];
    var frame = getEl('videoFrame');
    frame.src = 'https://player.bilibili.com/player.html?bvid=' + bvid + '&autoplay=0';
    frame.style.display = 'block';
